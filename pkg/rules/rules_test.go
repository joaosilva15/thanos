	...
	for _, tcase := range []struct {
		...
		t.Run(tcase.requestedType.String(), func(t *testing.T) {
			...
			for i := range got.Groups {
				for j, r := range got.Groups[i].Rules {
					if r.GetAlert() != nil {
						got.Groups[i].Rules[j] = someAlert
						continue
					}
					if r.GetRecording() != nil {
						got.Groups[i].Rules[j] = someRecording
						continue
					}
					t.Fatalf("Found rule in group %s that is neither recording not alert.", got.Groups[i].Name)
				}
				if len(got.Groups[i].Rules) == 0 {
					// Fix, for test purposes.
					got.Groups[i].Rules = nil
				}
				// Mask nondeterministic fields.
				got.Groups[i].EvaluationDurationSeconds = 0
				got.Groups[i].LastEvaluation = time.Time{}

				testutil.Equals(t, expectedForType[i], got.Groups[i])
			}
